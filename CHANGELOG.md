...
## [v0.7.7] — 2026-04-29 (D-124 素材库精品原片库)

- **产品**: 素材库从 Downloads 文件夹浏览器改成精品原片库心智, 首页固定 8 个业务大类:
  待整理、演讲舞台、上课教学、研发产品、出差商务、做课素材、空镜补画面、品牌资产.
- **后端**: `material_assets` V5 增加结构化画像字段, 新增 `/api/material-lib/categories`,
  `/match`, `/classify/{id}`, `/classify-batch`; 分类快速层基于文件名、路径、尺寸、时长和旧标签,
  不调用 LLM.
- **前端**: 素材库页新增素材源目录保存、业务大类货架、画面摘要/质量分/用途建议展示、剪辑检索,
  并补桌面/移动响应式布局.
- **安全**: 旧 LLM `tag-batch` 超 100 条必须显式确认, 避免 Downloads 上千文件误全量烧 credits.
- **测试**: materials 相关单测/集成测试扩展; `pytest -q` 通过; 临时 API curl 和 Playwright
  桌面/移动截图验证通过.

## [v0.7.6] — 2026-04-29 (D-101 热点改写多版本生成)

- **修复**: 热点改写 Step 2 文案写着“本次会出 2/4 篇”, 但后端实际只跑 1 篇.
- **后端**: `/api/hotrewrite/write` 新增 `modes` 入参和 `version_count` 返回;
  后台任务按勾选模式真生成 2/4 版, `task.result.versions[]` 保留每版正文、自检和 token.
- **前端**: 改写模式默认两种都勾, 默认 V1-V4 四版; 任务完成后一次展示多版,
  正文顶部/底部都可切换版本.
- **测试**: 新增 D-101 单测 + Playwright 回归; `pytest -q -x` 通过, 截图确认
  4 版切换无遮挡.

## [v0.7.5] — 2026-04-29 (D-100 做视频 Step 1 热点排行入口)

- **优化**: `?page=make` Step 1 默认页底部从“复用最近做过的”改成常驻“热点排行”.
- **流程**: 点击热点卡直接进入热点改写, 自动带入热点标题/匹配原因并进入拆解选角度.
- **界面**: 热点排行展示排名、平台、夜班来源、匹配度和热度条; 暂时隐藏底部 6 个
  skill 快捷卡, 降低默认页噪声.
- **验证**: Playwright 截图确认默认页只显示热点排行; 点击热点进入热点改写 Step 2,
  analyze 请求 body 含所选热点; console/pageerror=0.
...
- **修复**: Step 5 段间图 4/4 都已生成后, 页面只剩单张"重生"按钮,想重跑 4 张只能
  一张张点.
- **前端**: `pending=0 && doneCount>0` 时显示 `一键重生 N 张`; 点击后并发提交所有
  已完成卡片. 单张进入 running 时清掉旧图链接,避免重生中继续显示旧图.
- **测试**: 扩展 `scripts/e2e_wechat_d096_flow.js`, 确认 4/4 后"一键重生 4 张"会再
  并发提交 4 个 `/api/wechat/section-image` 请求.

## [v0.7.2] — 2026-04-28 (D-096 公众号全链路交互收口)

- **修复**: 公众号 Step 2/5/6 三处连环体验问题: 标题换批重复、段间配图自动开跑、
  拼 HTML 错误被二次包装成未知模式.
- **标题**: `/api/wechat/titles` 新增 `avoid_titles` / `round`; 前端换批时把上一批
  传给后端避重,后端也过滤完全重复标题.
- **段间配图**: Step 5 改成先选统一风格,再一键并发生成剩余图片; 单张生成同样必须
  先选风格. 切风格只重写提示词和清状态,不自动烧图.
- **HTML 拼装**: 公众号转换脚本自动选择装了 `bs4/premailer` 的系统 Python,避免后端
  `.venv` 缺依赖导致 500; 前端保留后端 detail 并显示公众号排版环境错误.
- **测试**: `pytest -q -x` 通过; `scripts/e2e_wechat_d096_flow.js` 覆盖标题换批、
  配图并发、HTML 错误提示; 真实 `/api/wechat/html` 返回 200.

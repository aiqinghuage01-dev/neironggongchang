...
**任何代码改动后，Claude 自己完成验证。不许把验证甩给用户。**

#### UI 改动（动 web/index.html 或前端逻辑）
- 必须用 `preview_screenshot` 截图证明效果
- 必须用 `preview_console_logs` 看 console，没有红色报错
- 改了交互必须用 `preview_click` / `preview_fill` 实际跑一遍
- **没截图 + 没看 console = 不许说"修了" / "完成" / "应该好了"**

#### 后端改动（动 backend/ 或 shortvideo/）
- 必须 `pytest -x` 相关测试
- 改了 API 必须 `curl` 一次真实请求看返回
- **没测 = 不许说"修了"**

#### 修 bug 额外规矩
- 修完必须加一条 pytest 回归测试（fail before fix → pass after）
- 测试和代码同 commit
- **没回归测试 = bug 不算修**

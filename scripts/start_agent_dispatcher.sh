...
label="com.neironggongchang.agent-dispatcher"
repo_root="$(git rev-parse --show-toplevel)"
plist="${HOME}/Library/LaunchAgents/${label}.plist"
log_file="/tmp/nrg-agent-dispatcher.log"
uid="$(id -u)"
python_bin=""
...
usage() {
  cat <<'USAGE'
Usage:
  bash scripts/start_agent_dispatcher.sh [options]

Options:
  --stop         Stop the background dispatcher.
...
if [[ "$mode" == "status" ]]; then
  ...
  cd "$repo_root"
  export PATH="$tool_path"
  "$python_bin" scripts/agent_dispatcher.py --status || true
  exit 0
fi

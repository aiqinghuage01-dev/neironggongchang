...
## 当前状态 (2026-05-01 · 写文案首页任务恢复收口)

**版本**: v0.8.22-write-active-summary — 写文案首页已能显示“正在写 / 可继续”任务摘要, 支持投流、热点、录音、爆款、内容策划、违规审查、公众号长文 7 类任务一键回跳。恢复时保留旧 workflow 上下文, 不再覆盖标题/大纲/转写/策划结果; 首页摘要扩展内部词与本机路径脱敏; 无时间戳 failed task 不再长期挂首页。自动派工器与自动返修主管继续运行。

### 当前进行
- T-101/T-110/T-111/T-112/T-113/T-114 已合入 main: 写文案首页任务摘要完成首轮功能、QA、Review 和 P2 polish。主线提交 `bd0eaeb`/`7802b7b`/`bc57d64`/`f28e339`/`c4156eb`; T-112 内容开发进程产出补丁后卡在未提交状态, 总控按 `worker_stuck` 接管收口; T-113/T-114 自动 QA/Review worktree 未包含 `f28e339`, 已停止过期验证并由总控按同等标准接管验证/复审。正式 `8000/8001` no-credit Playwright 覆盖 7 类回跳、旧 `wf:*` 快照保留、无时间 failed 隐藏、390px 移动端; console/pageerror/requestfailed/http error/nonGET API 全 0。另补派工器提示护栏: 新 worker prompt 会显示主工作区、主线 HEAD、当前 worktree 是否包含主线 HEAD, 避免 QA/Review 测旧代码。截图已读: `/tmp/_ui_shots/t101_write_active_tasks.png`, `/tmp/_ui_shots/t112_write_active_tasks_more.png`, `/tmp/_ui_shots/t112_resume_voicerewrite.png`, `/tmp/_ui_shots/t112_resume_planner.png`, `/tmp/_ui_shots/t112_resume_wechat.png`, `/tmp/_ui_shots/t112_write_untimed_failed_hidden.png`, `/tmp/_ui_shots/t101_write_active_tasks_390.png`。报告: `docs/agent-handoff/DEV_CONTENT_T112_WRITE_ACTIVE_POLISH_20260501.md`, `docs/agent-handoff/QA_T113_WRITE_ACTIVE_POLISH_20260501.md`, `docs/agent-handoff/REVIEW_T114_WRITE_ACTIVE_POLISH_20260501.md`, `docs/agent-handoff/CONTROLLER_T112_WRITE_ACTIVE_FINAL_20260501.md`。
...
- T-062 总控二轮全站文案清理已完成: 首页和写文案页最后两处 `AIGC` / `AI token` 可见工程词已改掉; 21 个路由页面工程词/本机路径/接口词扫描命中 0, console/pageerror/requestfailed/http error 全 0。
- T-063 独立 QA 已正确 block: 21 页只读扫描中知识库页面可见 `persona-prompt`, 命中禁止词; 报告 `docs/agent-handoff/QA_T063_SITE_COPY_READONLY_20260430.md`。
...
- T-048 独立 QA 已正确 block: 公众号仍可见 `HTML`, 录音改写仍可见 `URL/ASR/D-062bb-ext`; 报告 `docs/agent-handoff/QA_T-048_SITE_OPT_REGRESSION_20260430.md`。
- T-065 独立 QA 已正确 block: 主页面 21 页通过, 但 `beta` 页 iframe 嵌入内部研发工作台, frame-aware 扫描命中 `/Users/OpenClaw/DeepSeek/Opus/Downloads/404/500/504` 等; 报告 `docs/agent-handoff/QA_T065_SITE_COPY_REPAIR_20260430.md`。
- T-048/T-065 总控返修已完成: 公众号 `HTML` 口径统一为“排版”; 录音改写 `URL/ASR/D-code` 改成链接/自动转写/本地音频稍后接入; `beta` 页移除内部工作台 iframe, 改为本页原生安全摘要; 同步吸收 T-049 的失败态去重和朋友圈 InlineError 小修。报告 `docs/agent-handoff/CONTROLLER_T048_T065_COPY_IFRAME_REPAIR_20260430.md`。
...
- 投流快出链路已恢复并合入 main: T-021/T-022 报告显示页面真烧 `n=1` 只提交 1 次, task `2ca3ceaff54e481c8045573afc7cd50b` 34 秒 `ok`, `route_key=touliu.generate.quick`, `engine=opus`; 总控 cherry-pick `3ba6254`/`1384513` 并补 `205d109` 让 API 对 `n<=2` 返回 60 秒估时。T-054 已排队恢复录音改写 + 热点改写真实链路复测。
- T-043 审查无 P0, 但指出素材库 featured 缺失文件过滤和首页降级问题; 总控已返修: `missing_at IS NULL`, featured limit 422 校验, featured 失败不拖垮首页, 空 featured 显示识别引导, KPI 文案改为「质量达标」。
...
- T-057 报告: `docs/agent-handoff/CONTROLLER_T057_HOTREWRITE_FALLBACK_20260430.md`。
...
- T-062 总控报告: `docs/agent-handoff/CONTROLLER_T062_SITE_COPY_SWEEP_20260430.md`; targeted Playwright `home/write` 命中 0; 21 页全站 Playwright 扫描工程词、本机路径、接口词命中 0, console/pageerror/requestfailed/http error 全 0; 截图 `/tmp/_ui_shots/t062_site_scan_after_*.png`。

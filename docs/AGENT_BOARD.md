...
| 角色 | 状态 | 工作区 | 当前任务 |
|---|---|---|---|
| 总控 Agent | 进行中 | `~/Desktop/neironggongchang` | 审查 T-013/T-014 交接; T-015/T-016 已进入自动任务队列 |
...
| QA 测试 Agent | 自动领任务 | `~/Desktop/nrg-worktrees/qa`, `~/Desktop/nrg-worktrees/qa-1`, `~/Desktop/nrg-worktrees/qa-2` | 队列已有 T-015; T-016 依赖 T-015 通过 |
...
| T-015 | T-014 修后投流 `n=1` 最小真烧复测 | QA 测试 Agent | 队列中 | 不改功能代码; 只提交报告/必要测试脚本需总控确认 | 只提交 1 次投流 `n=1`; task `ok`; 页面/接口有结果; console/pageerror=0; 记录耗时和 AI usage; 不通过则不继续烧录音/热点 |

...
### A. 访客模式产出无地方留底
**症状**: 切访客模式后, 文字 skill / 公众号 / 段间图都不入作品库 (D-070 短路了 `_autoinsert_text_work` + `wechat_scripts insert_work`).
**当前**: 老板要保留朋友项目稿子, 得手动复制粘贴.
**Follow-up**: 加访客模式独立隔离 namespace 作品库, 用完能"导出/清空"整批. 一期可接受不做.

### B. 访客模式 AI 走中性写作助手, 不能"切朋友的人设"
**症状**: 访客模式下 AI 不注入"清华哥"几千字人设, 走 ~100 字的"中文写作助手" system prompt. 朋友项目稿子风格中性.
**Follow-up**: 多租户人设 — 给每个朋友项目配自己的 persona-prompt. 一期不做 (老板没要这层).

### C. 战略部页面是 placeholder
**症状**: D-068b 加的 `战略部` 入口点击进去只看到"等装战略规划技能"的占位页.
**计划**: 等老板装上战略规划相关 skill (类似 content-planner D-022 接入模式).

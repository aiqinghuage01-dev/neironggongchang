...
>
> **v1.2 增补(2026-05-01 · 首台 Mac mini Tailscale 实测回写):**
> - 新增 §0.8.9「第二台 Mac mini 复用 runbook」——把 `poju-mini` 实测迁移中的 Tailscale、SKILL_ROOT、空库、依赖、SSH tunnel 验收命令固化
> - 明确 GitHub clone 是首选,`gh auth`/token 不通时可用 `rsync` 临时绕过,但后续必须补齐 git pull 能力
> - 明确 `backend/web` 在 Mac mini 上应只监听 `127.0.0.1`,Tailscale IP 直连 8000/8001 失败是正确安全状态
>
> **v1.3 增补(2026-05-01 · skill 真实源/视图层边界收紧):**
> - 明确 Mac mini 上 `~/skills-source/` 是真实 skill 仓库,`~/Desktop/skills/团队版` 只是 symlink 视图
> - 明确迁移时只能 rsync 到 `~/skills-source/`,**禁止覆盖 Mac mini 的 `~/Desktop/skills/` 视图层**
> - 保留 `团队版 / 自用版 / 学员版` 三视图设计,避免未来迁移把分发架构扁平化
>
> **11 轮收敛轨迹:**
>
> | 版本 | 增量 | 总行数 | 主修 |
...
│  · ~/Desktop/skills/  ──▶ ~/skills-source/   │  (真实 skill 仓库)
│  · ~/Desktop/清华哥知识库/                    │
│  · ~/Desktop/我的内容库/                      │
│  · 精选素材                                  │
│                                              │
...
| 4 | rsync 开发机 `~/Desktop/skills/` 到 Mac mini `~/skills-source/`;知识库/精选素材按需同步 | Tailscale | M1 day 1 | 0.5 d(含 §1) |
...
2. **GitHub 永远不放敏感资产,即使是 Private 仓库**
   - `.env`、DB、媒体、`vendor/`、真实 skill 仓库、知识库、素材库——全部走 rsync,不走 git
   - 防 GitHub 泄漏 + 防 admin 切换设备时遗忘"这个目录是不是也得搬"
   - `.gitignore` 是技术保障,**默认不放**是流程保障

3. **Mac mini 的 `~/Desktop/skills/` 是视图层,不是 rsync 目标(v1.3 收紧)**
   - 真实存储:`~/skills-source/`
   - 团队工具读取:`~/Desktop/skills/团队版 -> ~/skills-source/`
   - 预留视图:`~/Desktop/skills/自用版`、`~/Desktop/skills/学员版`
   - **禁止**执行 `rsync ~/Desktop/skills/ <mac-mini>:~/Desktop/skills/`。这会把 `团队版` symlink 覆盖成普通目录,破坏三视图分发设计。

4. **步骤 5 → 6 之间是"内测 → 公网"的关键开关,不能跳过**
   - 如果没经过步骤 5(只 admin 内网验证),直接开步骤 6
   - 团队第一眼看到的就可能是坏的——浏览器登录态过期、CosyVoice 没启动、防火墙挡了某端口等
   - 步骤 5 是 admin 给自己留的"踩坑窗口",必须先踩完再上线

5. **rsync 不要 `--delete`(防误删)**
   - 第一次推荐:`rsync -avzP <src>/ <mac-mini>:<dst>/`(不删 Mac mini 端多出的文件)
   - 后续增量同步同上
   - 如果要 mirror,必须先在 Mac mini 上 `cp -R` 一份带时间戳的兜底

6. **私有资产清单(rsync 必搬,git 必不搬)**

   | 路径 | 大小估算 | 必要性 | rsync 频率 |
   |---|---|---|---|
   | 开发机 `~/Desktop/skills/` → Mac mini `~/skills-source/` | 数百 MB | 必须 | M1 一次性,后续按需 |
   | Mac mini `~/Desktop/skills/` | 小 | 必须保留 | **不 rsync,只维护 symlink 视图** |
   | `~/Desktop/清华哥知识库/` | GB 级 | 必须 | M1 一次性,后续日同步或周同步 |
   | `~/Desktop/我的内容库/` | GB 级 | 必须 | M1 一次性,后续按需 |
   | `data/` | 不定 | 干净迁移不搬 | 仅 fallback 完整迁移路线才搬 |
...
| | 开发机 `~/Desktop/skills/` → Mac mini `~/skills-source/` | rsync via Tailscale |
| | Mac mini `~/Desktop/skills/团队版` | symlink 到 `~/skills-source/`,不 rsync 覆盖 |
...
#### 0.8.9 第二台 Mac mini 复用 runbook(v1.2/v1.3 · 基于 poju-mini 实测)

> 2026-05-01 已在首台 Mac mini `poju-mini` 完成一轮真实验证。本节把实测命令和坑位写成复用清单,
> 以后换第二台 Mac mini 时照此跑,不要重新猜路径/依赖/端口。

**实测结论:**
- Tailscale MagicDNS `poju-mini` 可 SSH,`tailscale ping poju-mini` 约 8ms direct
- 项目可在 Mac mini 上用**全新空库**启动,`apply_migrations()` 后 `data/works.db` 有 17 张表,核心业务表 0 行
- `SKILL_ROOT=~/Desktop/skills/团队版` + symlink 到 `~/skills-source/` 可读统一 skill 仓库;公众号 skill 实测 `SKILL.md` 13994 字符、5 篇 references
- `backend` / `web` 临时服务均只监听 `127.0.0.1`,Tailscale IP 直连 `:8000/:8001` 被拒绝,这是正确安全状态
- 通过 SSH tunnel 打开 Mac mini 页面,`make` / `hotrewrite` console error=0,国家领导人相关词命中 0
...
# 开发机上
tailscale status
tailscale ping --timeout=5s <mac-mini-name>
ssh <mac-mini-name> 'hostname; whoami; pwd'
```

验收口径:
- `tailscale ping` 返回 `pong` 且显示 direct/relay 均可接受,direct 更好
- `ssh <mac-mini-name>` 能登录即可;直接 `ssh <tailscale-ip>` 可能因为用户名不匹配失败,不作为阻断
- 推荐使用 MagicDNS 名称(如本次 `poju-mini`)作为后续 rsync/ssh 目标
...
# Mac mini 上
cd ~/Desktop
git clone <private-repo-url> neironggongchang
cd neironggongchang
uv venv --python 3.12
uv sync --locked
```

如果 `gh auth` / token 暂时不被 Mac mini 接受,允许先用 `rsync` 绕过,但这只是 M1 准备期手段:

```bash
# 开发机上。如果 Mac mini 端已经 git clone 过,保留远端 .git,只覆盖工作区。
rsync -avz \
  --exclude '.git/' \
  --exclude '.venv/' \
  --exclude 'data/' \
  --exclude 'vendor/' \
  ~/Desktop/neironggongchang/ <mac-mini-name>:~/Desktop/neironggongchang/
```

硬约束:
- `rsync` 不能替代长期版本管理。团队上线前,Mac mini 必须能 `git pull` 或由明确的部署脚本更新。
- 如果 Mac mini 端还没有 `.git/`,不要排除 `.git/`;否则会得到一个不可 `git pull` 的普通目录。
- `data/` 走干净迁移路线,默认不 rsync 旧库。
- `.env` 不入库,在 Mac mini 上按 `.env.example` 手工新建。

**C. 运行时依赖 sanity**

本次实测发现缺 `jieba` 和 `python-multipart` 会让后端 import 阶段直接挂:
- `jieba`: `backend/services/kb.py` 运行时依赖
- `python-multipart`: FastAPI `UploadFile` / form 解析依赖

这两个已写入 `pyproject.toml` / `requirements.txt` / `uv.lock`。第二台 Mac mini 上必须用锁文件安装,并做 import sanity:
...
**D. SKILL_ROOT 与 skill symlink 视图**

Mac mini 上分两层:

```text
~/skills-source/              # 真实 skill 仓库,admin 改这里
~/Desktop/skills/             # 分发视图层,不要 rsync 覆盖
├── 团队版 -> ~/skills-source/
├── 自用版
└── 学员版
```
...
```bash
# Mac mini 上先准备目标目录
mkdir -p ~/Desktop/skills ~/skills-source ~/Desktop/skills/自用版 ~/Desktop/skills/学员版

# 开发机上把现有 skills 同步过去
rsync -avz ~/Desktop/skills/ <mac-mini-name>:~/skills-source/

# Mac mini 上建立团队版视图
if [ -e ~/Desktop/skills/团队版 ] && [ ! -L ~/Desktop/skills/团队版 ]; then
  echo "REFUSE: ~/Desktop/skills/团队版 已存在且不是 symlink,请先人工确认后再处理"
  exit 1
fi
ln -sfn ~/skills-source ~/Desktop/skills/团队版
```

禁止命令:

```bash
# ❌ 不要这样做,会覆盖 Mac mini 的 团队版/自用版/学员版 视图层
rsync -avz ~/Desktop/skills/ <mac-mini-name>:~/Desktop/skills/
```

Mac mini `.env`:

```dotenv
SKILL_ROOT=~/Desktop/skills/团队版
```
...
print(skill["root"])
print(len(skill["skill_md"]), len(skill["references"]))
PY

test -L ~/Desktop/skills/团队版
readlink ~/Desktop/skills/团队版
```

注意:
- 直接 import `skill_loader` 不一定自动加载 `.env`;手工 sanity 要先 `source .env`,或先 import 项目配置层。
- API 服务正常启动时会经过项目配置层,只要 `.env` 存在即可。
- 不同 Mac mini 用户名可以不同,不要写死 `/Users/black.chen/...`;以 `~` 和 env 为准。
- admin 后续只改 `~/skills-source/<slug>/...`;所有走 `~/Desktop/skills/团队版` 的工具会通过 symlink 自动看到最新内容。
...
set -a; source .env; set +a
.venv/bin/python - <<'PY'
from shortvideo.db import apply_migrations, get_connection
apply_migrations()
con = get_connection()
...
counts = {
    ...
    for name in ("works", "hot_topics", "materials", "tasks")
}
print("tables", tables)
print(counts)
PY
...
验收口径:
- 表数量应接近当前版本实测值(本次为 17 张表;未来 migration 增加时可以变多)
- `works/hot_topics/materials/tasks` 在干净迁移路线下应为 0 行

**F. Mac mini loopback smoke**
...
cd ~/Desktop/neironggongchang
set -a; source .env; set +a
nohup .venv/bin/python -m uvicorn backend.api:app \
  --host 127.0.0.1 --port 8000 --log-level warning \
  > /tmp/nrg-mini-api.log 2>&1 &

nohup python3 -m http.server 8001 --bind 127.0.0.1 -d web \
  > /tmp/nrg-mini-web.log 2>&1 &
```

Mac mini 本机验收:

```bash
curl -fsS http://127.0.0.1:8000/api/wechat/skill-info | head
curl -fsS 'http://127.0.0.1:8000/api/hot-topics?limit=3' | head
...
```bash
# 开发机上,这些应该失败或 connection refused
curl -m 3 http://<mac-mini-name>:8000/api/wechat/skill-info
curl -m 3 http://<tailscale-ip>:8000/api/wechat/skill-info
```

解释:后端和临时 web 只监听 `127.0.0.1`,Tailscale 直连失败说明没有绕过 Cloudflare Access / Caddy 暴露裸端口。

**G. SSH tunnel 临时浏览器验收**

团队域名还没接 Cloudflare 前,admin 可以临时用 SSH tunnel 验证:
...
```text
http://127.0.0.1:18081/?page=make
```

开发控制台执行:

```js
localStorage.setItem("api_base", "http://127.0.0.1:18080")
location.reload()
```
...
- `make` / `hotrewrite` 能加载
- console error=0
- Network API 请求走 `127.0.0.1:18080`
- 页面不出现国家领导人相关词

验收后关闭 tunnel:

```bash
lsof -tiTCP:18080 -sTCP:LISTEN | xargs kill
lsof -tiTCP:18081 -sTCP:LISTEN | xargs kill
```

**H. 什么时候才算"通过域名能看"**

以上 A-G 只证明 Mac mini 作为目标机器已经能跑。要通过正式域名访问,还必须完成 M1 网络层:
- Caddy 监听 `127.0.0.1:8080`
- cloudflared 指向 `http://127.0.0.1:8080`
- Cloudflare Access 白名单开启
- `APP_ENV=prod` / `ALLOWED_ORIGIN=https://gongchang.poju.ai`

没有这 4 件事,就只能通过 SSH tunnel 本人调试,团队还不能用域名访问。
...
**END · v1.3 · 2026-05-01 · 已补 skill 真实源/视图层边界 · 可复用到下一台 Mac mini**

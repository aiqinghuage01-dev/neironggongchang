...
默认流程:
- 总控把页面变更拆给对应开发 Agent, 同时或随后派 QA 任务。
- 开发 Agent 可以标记开发完成, 但总控不能把业务目标说成完成, 直到 QA 通过。
- QA 至少提供: 变更后截图、console/pageerror/requestfailed/http error 统计、真实点击/填写/切换证据。
- 涉及响应式布局、卡片/表格/弹层/侧边栏时, QA 必须覆盖桌面和一个窄屏视口。
- 涉及后端数据或 API 时, QA 还要补 curl 或接口返回证据。

总控直接改页面只允许用于紧急止血、跨模块收口、worker 卡死或最终小修。即便如此, 总控也必须交付同等级 QA 证据; 如果接管了 `content` / `media` / `qa` / `review` 任务, 还必须写 `--takeover-reason`。

没有截图/console/真实操作证据时, 只能说“开发已改”, 不能说“页面完成”。

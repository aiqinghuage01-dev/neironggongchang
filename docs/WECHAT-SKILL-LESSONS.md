...
风格,暖色调", 但每段 prompt 自带的叙事氛围各异 → 真生图时模型按叙事走, 4 张视觉
风格不一致.

**修法 (前端层)**: `WxStepImages` 顶部加全局"🎨 统一风格" chip group, 6 个 PRESET.
切风格时给 4 张 strip 旧 append + 套新 append + 清状态触发自动重生. 用 prompt
末尾 append 强制风格关键词 (apimart 模型对末尾权重高), 盖过 LLM 隐含氛围.

**为什么不在后端 plan_section_images 里加风格参数**: 前端 prompt 末尾 append 已经
够用 (实测有效), 改后端 schema + LLM prompt 引入更大改动面. 老板需求 = "选个风格
4 张统一", 前端 strip+append 一致即可, 不必等 LLM 重出.

**幂等保证**: `stripStyleAppendsAtEnd` 多扫 2 轮 strip 末尾任何已知 PRESET.append,
防"全局 append + 单张 chip append"双层. 切换 N 次风格不会累积.

**localStorage 偏好**: `wechat:section_image:global_style` 持久化, 老板下次进来
还是上次选的风格.

**测试入口**: 没 JS 测试 infra, playwright 端到端 (`/tmp/_d091_style_loop.js` 模板).
注入 `wf:wechat` snapshot 给 4 张 pending plan → 验默认 real append → 切 ink →
验 prompt 切换 + localStorage 写入.

---

## 8. 改完必跑的闭环 (CLAUDE.md 完工铁律)

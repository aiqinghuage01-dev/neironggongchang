...
**2026-04-29 补充 · 精品原片库方向**:
素材库的核心不是管理电脑里的所有文件, 而是服务未来短视频剪辑。素材主要来自清华哥实际拍摄的照片和视频原片; 剪辑时命中文案语义后, 优先从本地素材库找真实画面, 找不到再 AI 生成。当前先用 `~/Downloads/` 作为演示数据源跑通识别、分类、展示、检索; 未来切到 `~/Desktop/清华哥素材库/` 等专用文件夹, 只放 50-200 个精品素材。
...
每条真实素材需要沉淀画面摘要、标签、业务大类、镜头类型、横竖屏、质量分、适合用途, 供后续剪辑检索。
